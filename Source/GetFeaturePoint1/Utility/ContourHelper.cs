    ...
    public class ContourHelper
    {
        ...
        public static void UnifyDirectionOfContour(IFeatureClass fc)
        {
            ...
            while (fea != null)
            {
                ...
                {
                    IRow Row = table.GetRow(fea.OID);
                    object Elev = Row.get_Value(2);
                    double maxElev = Convert.ToDouble(Elev);
                    int feaOid;
                    if (Convert.ToDouble(Elev) > maxElev)
                    {
                        maxElev = Convert.ToDouble(Elev);
                        feaOid = fea.OID;
                    }
                }
            }
